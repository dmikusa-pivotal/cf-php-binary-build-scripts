...
cd mod_fcgid-2.3.7
make clean
APXS=/tmp/staged/app/httpd/bin/apxs 
./configure.apxs 
make
make install
cd ../
...
sed 's/\/tmp\/staged\/app/\${HOME}/g' apachectl > apachectl
sed 's/\/tmp\/staged\/app/\${HOME}/g' apr-1-config > apr-1-config
sed 's/\/tmp\/staged\/app/\${HOME}/g' apu-1-config > apu-1-config
sed 's/\/tmp\/staged\/app/\${HOME}/g' envvars > envvars
sed 's/\/tmp\/staged\/app/\${HOME}/g' envvars-std > envvars-std
# switch single to double quotes so variable expansion occurs
sed -r "s/HTTPD='(.*)'/HTTPD=\"\1\"/g" apachectl > apachectl
